...
% radio_channel_wave_signal,  input time sequence 
    ...
    radio_channel_symbol_signal = fft(radio_channel_symbol_signal, FFT_SIZE, 1);
       
    v_ssb_case = glo_ssb.param.ssb_case; 
    
    switch v_ssb_case
        case "CASE_A"
            ssb_grid_list = ssb_demapping_case_a(radio_channel_symbol_signal);
            
        case "CASE_B"
            ssb_grid_list = ssb_demapping_case_b(radio_channel_symbol_signal);
...
%  ssb_grid , three dimension,  [FFT_DATA(4096), symbol(ssb length), SSB num]
